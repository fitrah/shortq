...
export default function DashboardClient({ initialLinks }: { initialLinks: LinkItem[] }) {
  ...
  return <main className="mx-auto max-w-7xl px-5 py-10"><section><p className="text-sm font-bold text-cyan-400">SHORT LINKS</p><h1 className="text-4xl font-black">Kelola tautan</h1><p className="mt-2 text-slate-400">Alias, proteksi password, status, dan masa berlaku dalam satu tempat.</p></section>
    <form onSubmit={create} className="mt-8 grid gap-3 rounded-2xl border border-slate-800 bg-slate-900 p-5 md:grid-cols-2 lg:grid-cols-4">
      ...
      <input name="password" type="password" minLength={4} placeholder="Password opsional" className={inputClass} />
      <label className="text-xs text-slate-400">Kedaluwarsa<input name="expiresAt" type="datetime-local" className={`mt-1 w-full ${inputClass}`} /></label>
    ...
    {editing && <div className="fixed inset-0 z-20 grid place-items-center bg-black/70 p-4"><form onSubmit={save} className="w-full max-w-xl space-y-3 rounded-2xl border border-slate-700 bg-slate-900 p-6"><div className="flex justify-between"><h2 className="text-xl font-bold">Edit /{editing.alias}</h2><button type="button" onClick={() => setEditing(null)}>✕</button></div><input required name="targetUrl" type="url" defaultValue={editing.targetUrl} className={`w-full ${inputClass}`} /><input required name="alias" defaultValue={editing.alias} className={`w-full ${inputClass}`} /><input name="title" defaultValue={editing.title || ''} placeholder="Judul" className={`w-full ${inputClass}`} /><input name="password" type="password" minLength={4} placeholder="Password baru (kosong = tidak berubah)" className={`w-full ${inputClass}`} /><label className="block text-sm"><input name="removePassword" type="checkbox" /> Hapus password</label><label className="block text-sm"><input name="isActive" type="checkbox" defaultChecked={editing.isActive} /> Aktif</label><label className="block text-sm text-slate-400">Kedaluwarsa<input name="expiresAt" type="datetime-local" defaultValue={editing.expiresAt ? editing.expiresAt.slice(0, 16) : ''} className={`mt-1 w-full ${inputClass}`} /></label><button disabled={busy} className="w-full rounded-xl bg-cyan-400 p-3 font-bold text-slate-950">Simpan perubahan</button></form></div>}
  </main>;
}
